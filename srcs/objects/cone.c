...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/21 16:54:44 by mmanaoui          #+#    #+#             */
/*   Updated: 2025/04/21 16:54:45 by mmanaoui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
							...
							t_vector ray, t_eq body, t_vector origin)
{
	...
	if (!get_cone_caps(&cap, obj, ray, origin)
									...
									t_vector ray, t_eq eq, t_vector origin)
{
	...
	if (eq.dist1 <= MIN_DIST && eq.dist2 <= MIN_DIST)
		return (0);
	eq.m1 = dot_product(ray, obj->normalize) * eq.dist1
		+ dot_product(vector_substract(origin, obj->center), obj->normalize);
	eq.m2 = dot_product(ray, obj->normalize) * eq.dist2
		+ dot_product(vector_substract(origin, obj->center), obj->normalize);
...
float	cone_intersect(t_vector origin, t_object *obj, t_vector ray)
{
	...
	obj->hit.is_inside = 0;
	orig_co = vector_substract(origin, obj->center);
	normalization(&obj->normalize);
	dot_r_nv = dot_product(ray, obj->normalize);
	dot_cc_nv = dot_product(orig_co, obj->normalize);
	tan_ha = 1 + tan(obj->radius / obj->height)
		* tan(obj->radius / obj->height);
	eq.a = dot_product(ray, ray) - tan_ha * dot_r_nv * dot_r_nv;
	eq.b = 2 * (dot_product(ray, orig_co) - tan_ha * dot_r_nv * dot_cc_nv);
	eq.c = dot_product(orig_co, orig_co) - tan_ha * dot_cc_nv * dot_cc_nv;
