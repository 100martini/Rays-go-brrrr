...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/21 16:54:51 by mmanaoui          #+#    #+#             */
/*   Updated: 2025/04/21 16:54:52 by mmanaoui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	cylinder_intersect(t_vector origin, t_object *obj, t_vector ray)
{
	...
	obj->hit.is_inside = 0;
	orig_cyl = vector_substract(origin, obj->center);
	normalization(&obj->normalize);
	dot_r_nv = dot_product(ray, obj->normalize);
	dot_cc_nv = dot_product(orig_cyl, obj->normalize);
	eq.a = dot_product(ray, ray)
		- dot_r_nv * dot_r_nv;
	eq.b = 2 * (dot_product(ray, orig_cyl) - dot_r_nv * dot_cc_nv);
	eq.c = dot_product(orig_cyl, orig_cyl)
