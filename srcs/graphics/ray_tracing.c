...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/21 16:54:02 by mmanaoui          #+#    #+#             */
/*   Updated: 2025/04/21 16:54:03 by mmanaoui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ray_tracing(t_minirt *minirt)
{
	...
	while (proj.y_angle >= (WIN_HEIGHT / 2.0) * -1.0)
	{
		...
		while (proj.x_angle <= (WIN_WIDTH / 2.0))
		{
			...
			ray = new_vector(x_ray, y_ray, -1.0);
			normalization(&ray);
			object_intersects(minirt, minirt->scene.objs, ray, &point);
			pixel_put(minirt, proj.mlx_x++, proj.mlx_y, point.color);
		}
		proj.mlx_y++;
	}
}
